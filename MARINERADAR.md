...
- **Aşama 34 (TAMAMLANDI ✅):** Üst Menü "Login / My Fleet" Modalı & Gemi Ekle/Çıkar Yönetimi geliştirildi (`public/index.html`, `public/app.js`, `src/controllers/userController.js`, `src/routes/userRoutes.js`). Sayfa ortasındaki bant kaldırılarak Swagger butonunun yanına yeşil **"Login / My Fleet"** butonu yerleştirildi. Açılan modal üzerinden kullanıcı değişimi (User A / User B / Guest View), kullanıcının yetkili gemilerini listeleme, filoya yeni gemi ekleme (`POST /api/users/:id/vessels`) ve filodan gemi çıkarma (`DELETE /api/users/:id/vessels/:vesselId`) işlevleri bağlandı.
